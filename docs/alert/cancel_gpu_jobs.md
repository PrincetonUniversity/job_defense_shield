...
This alert assumes that you can query the Prometheus server. Make sure the Python code on the Jobstats website under [Analyzing the Prometheus Data](https://princetonuniversity.github.io/jobstats/setup/summaries/#analyzing-the-prometheus-data) works for you. Also, try running with `verbose: True` in the global settings of your YAML configuration file for more information.
